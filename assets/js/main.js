...
function ensureHeaderCounterStyles() {
  ...
  style.textContent = `
    ...
    .order-modal-backdrop.is-active {
      opacity: 1;
      visibility: visible;
    }
    .order-modal-box {
    ...
    .order-modal-backdrop.is-active .order-modal-box {
...
function addToCart(product) {
  ...
  const quantityToAdd = Math.max(1, parseInt(product.qty || 1, 10));

  if (existing) {
    existing.qty = (parseInt(existing.qty, 10) || 0) + quantityToAdd;
  } else {
...
function setupDetailsAddToCart() {
  const detailsSection = document.querySelector('.details');
  if (!detailsSection) return;

  const addBtn = detailsSection.querySelector('.button[href="cart.html"], .details__content .button');
  if (!addBtn) return;

  addBtn.addEventListener('click', (e) => {
    e.preventDefault();

    const title = detailsSection.querySelector('h2')?.textContent.trim() || 'Product';
    const priceText = detailsSection.querySelector('h3')?.textContent || '$0';
    const price = parseFloat(priceText.replace(/[^0-9.]/g, '')) || 0;
    const img = detailsSection.querySelector('#main-product-img')?.getAttribute('src') || 'assets/img/product-1-1.jpg';
    const qtyInput = detailsSection.querySelector('.quantity input');
...
/*=============== CHECKOUT PROCESS & PLACE ORDER ===============*/
function setupCheckoutPage() {
  const checkoutSection = document.querySelector('.checkout');
  if (!checkoutSection) return;

  const summary = checkoutSection.querySelector('.summary');
  const placeOrderBtn = summary?.querySelector('.button');
  if (!summary || !placeOrderBtn) return;

  const cart = getCart();

  // Actualizare listă produse în sumarul comenzii dacă există produse în coș
  if (cart.length > 0) {
    const subtotal = cart.reduce((sum, item) => sum + item.price * item.qty, 0);
    const paymentBox = summary.querySelector('.payment-box');

    const itemsHtml = cart
      .map(
        (item) =>
          `<p><span>${item.title} (x${item.qty})</span><strong>${money(item.price * item.qty)}</strong></p>`
      )
      .join('');
    ...
    summary.innerHTML = `
      ...
      <div class="payment-box">
        ...
        <img src="assets/img/banktransfer.png" alt="Bank Transfer">
      </div>
      <button class="button" id="place-order-btn">Place Order</button>
    `;
  }

  const btn = document.getElementById('place-order-btn') || placeOrderBtn;
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div class="order-modal-box">
          <div class="order-modal-icon"><i class="ri-checkbox-circle-fill"></i></div>
          <h3>Comanda a fost trimisă cu succes!</h3>
          <p>Îți mulțumim pentru cumpărături! Numărul comenzii tale este <strong>#EC-${orderNumber}</strong>. Vei primi curând confirmarea pe e-mail.</p>
          <a href="shop.html" class="order-modal-btn">Continuă cumpărăturile</a>
        </div>
      `;
