...
function updateCartCount() {
  ...
  document.querySelectorAll('a[href="cart.html"]').forEach((link) => {
    link.setAttribute('data-cart-count', totalQty);
...
function setupPriceFilter() {
  ...
  const productsContainer = document.querySelector('.shop .products__container');
  if (!range || !productsContainer) return;

  const productCards = Array.from(productsContainer.querySelectorAll('.product__card'));
  const shopTopText = document.querySelector('.shop__top p');
  ...
  if (!priceLabel) {
    ...
    range.insertAdjacentElement('afterend', priceLabel);
  }
  ...
  const applyFilter = () => {
    ...
    priceLabel.textContent = `Max price: ${money(maxPrice)}`;
    if (shopTopText) {
      shopTopText.textContent = `Showing ${visibleCount} product${visibleCount === 1 ? '' : 's'}`;
    }
  };
...
function updateWishlistCount() {
  ...
  document.querySelectorAll('a[href="wishlist.html"]').forEach((link) => {
    link.setAttribute('data-wishlist-count', count);
